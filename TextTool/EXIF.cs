...
namespace Tools.TextTool
{
	public class EXIF
	{
		public static System.Data.DataSet getEXIFdata(string fileName)
		{
			...
			if (FileTool.isIMG_extension(fileName))
			{
				try
				{
				...
				{
					FileTool.writeLog("ERR OPEN IMAGE : " + fileName + "\r\n" + ex.ToString());
					return Result;
				}
				...
				if (propItems != null && propItems.Length > 0)
				{
					...
					for (int i = 0; i < propItems.Length; i++)
					{
						...
						propName = getPropertyName(labels, "0x" + System.Convert.ToString(propItems[i].Id, 16).PadLeft(4, '0').ToUpper());
...
YCbCrPositioning|0x0213";
			...
			string[] property_arr = propertyList.Replace("\r\n", "").Split(',');
			...
			{
				DT.Rows.Add();
				DT.Rows[i][0] = property_arr[i].Split('|')[1];
				DT.Rows[i][1] = property_arr[i].Split('|')[0];
			}
